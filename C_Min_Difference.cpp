...
int main()
{
    ...
    for0(i, b, 1)
    {
        ll lb = lower_bound(arrA, arrA + a, arrB[i]) - arrA;
        if (lb == a)
            lb--;
        mn = min(mn, abs(arrA[lb] - arrB[i]));
    }
    for0(i, a, 1)
    {
        ll lb = lower_bound(arrB, arrB + b, arrA[i]) - arrB;
        if (lb == b)
            lb--;
        // see2(arrA[i], lb);
        mn = min(mn, abs(arrB[lb] - arrA[i]));
    }
    cout << mn << endl;
    return 0;
}